...
let DOM = {
  ...
  wantsNewTab(e) {
    ...
    let isTargetNamedTab =
      e.target.hasAttribute('target') && !e.target.getAttribute('target').startsWith('_')
  ...
  findExistingParentCIDs(node, cids) {
    ...
    cids.forEach((cid) => {
      this.filterWithinSameLiveView(this.all(node, `[${PHX_COMPONENT}="${cid}"]`), node).forEach(
        (parent) => {
          parentCids.add(cid)
  ...
  dispatchEvent(target, name, opts = {}) {
    ...
    let event =
      name === 'click' ? new MouseEvent('click', eventOpts) : new CustomEvent(name, eventOpts)
