...
describe('View + DOM', function () {
  ...
  test('formsForRecovery', function () {
    ...
    expect(newForms[0][0].getAttribute('phx-change')).toBe(
  ...
  describe('phx-update', function () {
    let childIds = () =>
      Array.from(document.getElementById('list').children).map((child) => parseInt(child.id))
...
describe('View + Component', function () {
  ...
  test('pushInput', function (done) {
    expect.assertions(6)
    let html = `<form id="form" phx-change="validate">
    ...
    window.requestAnimationFrame(() => {
      expect(
        el
          .querySelector(`[phx-feedback-for="${first_name.name}"`)
          .classList.contains('phx-no-feedback')
      ).toBeFalsy()
      expect(
        el
          .querySelector(`[phx-feedback-for="${last_name.name}"`)
          .classList.contains('phx-no-feedback')
      ).toBeTruthy()
  ...
  test('pushInput sets phx-no-feedback on feedback groups', function (done) {
    ...
    window.requestAnimationFrame(() => {
      expect(
        el
          .querySelector(`[phx-feedback-for="${email.name}"]`)
          .classList.contains('phx-no-feedback')
      ).toBeFalsy()
      expect(
        el
      ...
      expect(
        el.querySelector('[phx-feedback-for="mygroup"]').classList.contains('phx-no-feedback')
      ).toBeTruthy()
  ...
  test('pushInput sets phx-no-feedback class on feedback elements for multiple select', function (done) {
    ...
    window.requestAnimationFrame(() => {
      expect(
        el
          .querySelector(`span[phx-feedback-for="user[first_name]"`)
          .classList.contains('phx-no-feedback')
      ).toBeFalsy()
      expect(
        el
          .querySelector(`span[phx-feedback-for="user[allergies]"`)
          .classList.contains('phx-no-feedback')
      ).toBeTruthy()
  ...
  describe('undoRefs', () => {
    ...
    test('triggers beforeUpdate and updated hooks', () => {
      ...
      view.update(
        {
          s: [
            '<span id="myhook" data-phx-ref="1" data-phx-ref-src="container" phx-hook="MyHook">Hello</span>',
          ],
        },
        []
      )
